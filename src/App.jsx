...
function App() {
  ...
  const [evidenceLocker, setEvidenceLocker] = useState([]);
  ...
  const BACKEND_URL = 'https://crypto-recon-backend-2.onrender.com';
  ...
  const saveCase = (reportData) => {
    const history = JSON.parse(localStorage.getItem('caseHistory') || '[]');
    const newCase = { id: caseId, wallet: reportData.wallet_address, risk: reportData.risk_score, timestamp: new Date().toLocaleString() };
    history.unshift(newCase);
    localStorage.setItem('caseHistory', JSON.stringify(history));
    setCaseHistory(history);
  };

  const addToLocker = (node) => {
    if (!evidenceLocker.find(e => e.id === node.id)) {
      setEvidenceLocker([...evidenceLocker, node]);
      setReportLog(`🔒 Evidence added: ${node.id.slice(0, 10)}...`);
    }
  };

  const removeFromLocker = (nodeId) => {
    setEvidenceLocker(evidenceLocker.filter(e => e.id !== nodeId));
  };

  const getChainTrail = () => {
    if (!report?.fraud_analysis) return [];
    const trail = [];
    if (report.classification === 'SUSPECT') trail.push('🚨 SUSPECT');
    if (report.fraud_analysis.bridges?.length > 0) trail.push('🌉 BRIDGE');
    if (report.fraud_analysis.mixers?.length > 0) trail.push('🌀 MIXER');
    if (report.vasp_details?.exchange !== 'NOT LINKED') trail.push('✅ VASP');
    if (trail.length === 0 && report.classification) trail.push(report.classification);
    return trail;
  };
  ...
  const traceWallet = async (address) => {
    ...
    setIsLoading(true);
    setFreezeStatus("");
    setReportLog(`🔍 Tracing ${address.slice(0, 10)}...`);
    try {
      ...
      saveCase(data.report);
    } catch (e) {
      setReportLog("❌ Backend failed");
    }
    setIsLoading(false);
  };
  ...
  const handleFileUpload = async (file) => {
    ...
    reader.onload = async (e) => {
      ...
      setReportLog(`📤 Processing ${addresses.length} wallets...`);
      const results = [];
      ...
      setReportLog(`✅ Batch complete! ${results.length} wallets processed`);
    };
  ...
  const handleDrop = (e) => {
    ...
    if (file && file.name.endsWith('.csv')) {
      ...
      setReportLog("⚠️ Sirf CSV file daalo!");
    }
  };
  ...
  const getTypologyBadges = () => {
    if (!report?.fraud_analysis) return [];
    const badges = [];
    if (report.classification === 'SUSPECT') badges.push({ label: '🚨 PHISHING', color: '#ff0044' });
    if (report.fraud_analysis.bridges?.length > 0) badges.push({ label: '🌉 BRIDGE', color: '#00ccff' });
    if (report.fraud_analysis.mixers?.length > 0) badges.push({ label: '🌀 MIXER', color: '#ff0044' });
    if (report.fraud_analysis.total_clusters > 0) badges.push({ label: '🧠 LAYERING', color: '#9C27B0' });
    if (report.classification === 'CLEAN_INACTIVE') badges.push({ label: '✅ CLEAN', color: '#00ff88' });
    if (badges.length === 0) badges.push({ label: '❓ UNKNOWN', color: '#78909c' });
    return badges;
  };
    ...
    <div style={{ height: '100vh', width: '100vw', background: 'linear-gradient(135deg, #0a0a0f 0%, #1a1a2e 50%, #0a0a0f 100%)', color: 'white', fontFamily: "'Segoe UI', sans-serif", display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', zIndex: 1 }}>
        <div style={{ width: '34%', background: 'rgba(20,20,35,0.9)', padding: '15px', borderRight: '1px solid rgba(255,255,255,0.1)', overflowY: 'auto' }}>
          ...
          {activeTab === 'trace' ? (
            <>
              ...
              <button onClick={() => traceWallet(searchAddress)} disabled={isLoading} style={{ width: '100%', padding: '12px', background: isLoading ? '#444' : 'linear-gradient(135deg, #ff0044, #cc0033)', color: 'white', fontSize: '13px', fontWeight: 'bold', border: 'none', cursor: isLoading ? 'not-allowed' : 'pointer', borderRadius: '8px', marginBottom: '10px' }}>{isLoading ? '⏳ AI TRACING...' : '🚨 AI TRACE'}</button>
              ...
              <div 
                ...
                style={{ padding: '20px', border: `2px dashed ${isDragging ? '#00ff88' : '#444'}`, borderRadius: '8px', textAlign: 'center', marginBottom: '10px', cursor: 'pointer', background: isDragging ? 'rgba(0,255,136,0.1)' : 'transparent' }}
              >
                <p style={{ fontSize: '11px', color: '#888', margin: '0 0 5px 0' }}>📤 CSV file drag & drop karo</p>
                <p style={{ fontSize: '9px', color: '#00ff88', margin: 0 }}>Browse File</p>
                <input type="file" accept=".csv" ref={fileInputRef} onChange={(e) => handleFileUpload(e.target.files[0])} style={{ display: 'none' }} />
              </div>

              {batchResults.length > 0 && (
                <div style={{ padding: '10px', background: 'rgba(0,0,0,0.3)', borderRadius: '8px', fontSize: '10px', marginBottom: '10px' }}>
                  <p style={{ color: '#00ff88', fontWeight: 'bold', margin: '0 0 5px 0' }}>📊 BATCH RESULTS ({batchResults.length}):</p>
                  {batchResults.map((r, i) => (
                    <div key={i} style={{ padding: '5px', borderBottom: '1px solid #333' }}>
                      <span style={{ color: '#ccc' }}>{r.wallet_address?.slice(0, 10)}...</span>
                      <span style={{ color: r.risk_score > 80 ? '#ff0044' : '#00ff88', marginLeft: '10px' }}>Risk: {r.risk_score}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              <button onClick={() => { setSearchAddress("1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa"); traceWallet("1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa"); }} style={{ width: '100%', padding: '8px', marginBottom: '10px', background: 'rgba(0,255,136,0.2)', color: '#00ff88', border: '1px solid rgba(0,255,136,0.3)', borderRadius: '4px', cursor: 'pointer', fontSize: '10px' }}>₿ Satoshi Nakamoto (Demo)</button>
            </>
          ) : activeTab === 'suspects' ? (
            <>
          ...
          {caseHistory.length > 0 && (
            <div style={{ padding: '10px', background: 'rgba(0,0,0,0.3)', borderRadius: '8px', marginBottom: '10px' }}>
              <h4 style={{ fontSize: '11px', color: '#00ff88', margin: '0 0 5px 0' }}>📂 PAST CASES:</h4>
              {caseHistory.slice(0, 5).map((c, i) => (
                <div key={i} style={{ padding: '5px', borderBottom: '1px solid #333', cursor: 'pointer' }} onClick={() => traceWallet(c.wallet)}>
                  <span style={{ fontSize: '9px', color: '#ccc' }}>{c.wallet?.slice(0, 15)}...</span>
                  <span style={{ fontSize: '9px', color: c.risk > 80 ? '#ff0044' : '#00ff88', marginLeft: '10px' }}>Risk: {c.risk}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {evidenceLocker.length > 0 && (
            <div style={{ padding: '10px', background: 'rgba(156,39,176,0.1)', borderRadius: '8px', marginBottom: '10px' }}>
              <h4 style={{ fontSize: '11px', color: '#9C27B0', margin: '0 0 5px 0' }}>🔒 EVIDENCE LOCKER ({evidenceLocker.length}):</h4>
              {evidenceLocker.map((e) => (
                <div key={e.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px', borderBottom: '1px solid #333' }}>
                  <span style={{ fontSize: '9px', color: '#ccc' }}>{e.id?.slice(0, 15)}...</span>
                  <button onClick={() => removeFromLocker(e.id)} style={{ background: 'none', border: 'none', color: '#ff0044', cursor: 'pointer', fontSize: '10px' }}>✕</button>
                </div>
              ))}
            </div>
          )}
          ...
          {report && (
            <>
              <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginBottom: '10px' }}>
                {getTypologyBadges().map((badge, i) => (
                  <span key={i} style={{ padding: '5px 10px', background: badge.color, color: 'white', borderRadius: '12px', fontSize: '9px', fontWeight: 'bold' }}>{badge.label}</span>
                ))}
              </div>
          ...
          <div style={{ padding: '10px', background: 'rgba(0,0,0,0.5)', borderLeft: '4px solid #00ff88', borderRadius: '0 8px 8px 0' }}>
            ...
            {report?.fraud_analysis && (
              <div style={{ marginTop: '8px', padding: '8px', background: 'rgba(255,165,0,0.1)', borderRadius: '6px' }}>
                ...
                {report.fraud_analysis.clusters?.length > 0 && (
                  <div style={{ marginTop: '4px' }}>
                    {report.fraud_analysis.clusters.map((c, i) => (
                      <p key={i} style={{ fontSize: '9px', color: '#9C27B0', margin: '2px 0' }}>• {c.pattern} - {c.description}</p>
                    ))}
                  </div>
                )}
              </div>
        ...
        <div style={{ flex: 1, background: 'radial-gradient(circle, #1a1a2e 0%, #0a0a0f 100%)', position: 'relative' }}>
          {report && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '8px 15px', background: 'rgba(0,0,0,0.7)', borderBottom: '1px solid rgba(255,255,255,0.1)', position: 'absolute', top: 0, left: 0, right: 0, zIndex: 5, flexWrap: 'wrap' }}>
              {getChainTrail().map((step, i) => (
                <React.Fragment key={i}>
                  {i > 0 && <span style={{ color: '#00ff88' }}>→</span>}
                  <span style={{ fontSize: '10px', fontWeight: 'bold', color: '#FFA500' }}>{step}</span>
                </React.Fragment>
              ))}
            </div>
          )}
          ...
          {nodes.length > 0 && (
            <div style={{ display: 'flex', gap: '5px', position: 'absolute', top: report ? '45px' : '15px', left: '15px', zIndex: 10, flexWrap: 'wrap' }}>
              ...
              <button onClick={() => setViewType('SANKEY')} style={btnStyle('SANKEY')}>📊 SANKEY</button>
            </div>
          )}
          ...
          {nodes.length === 0 ? (
            ...
            viewType === '2D' ? <ReactFlow nodes={nodes} edges={edges} fitView onNodeClick={(e, node) => addToLocker(node)}><MiniMap style={{background:'#1a1a2e'}}/><Controls/><Background color="#333" gap={20}/></ReactFlow> :
            viewType === 'TREE' ? (
              <div style={{ padding: '30px', overflowY: 'auto', height: '100%' }}>
                {nodes.map((n, i) => <div key={n.id} onClick={() => addToLocker(n)} style={{ marginLeft: `${i*25}px`, padding: '10px', borderLeft: '3px solid #00ff88', cursor: 'pointer' }}><span style={{color: n.style?.background}}>●</span> {n.data?.label}</div>)}
              </div>
            ) : viewType === 'TIMELINE' ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '10px', padding: '30px' }}>
                {nodes.map(n => <div key={n.id} onClick={() => addToLocker(n)} style={{ padding: '20px', background: n.style?.background, borderRadius: '10px', textAlign: 'center', cursor: 'pointer' }}>{n.data?.label}</div>)}
              </div>
            )}
        </div>
      </div>
